...
public class FXMLController implements Initializable {
    ...
    private void End(ActionEvent event) throws Exception{
        if(mysql.insertToMatch(randomInt(),cBox1.getValue().toString(), cBox2.getValue().toString(), "Private", random_code.getText())){
            lab.setVisible(true);
    ...
    private void publicMatch(Event e ) throws IOException { 
        ...
        if (cBox1.getValue()==null || date.getValue()== null
            ...
            alert.showAndWait();
            System.out.println("الرجاء إكمال ملء الحقول السابقة");
        }else if(mysql.insertToMatch(randomInt(),cBox1.getValue().toString(), cBox2.getValue().toString(), "Private", random_code.getText()))
